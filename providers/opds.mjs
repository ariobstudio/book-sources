...
function basic(value) {
  const bytes = new TextEncoder().encode(value); const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'; let output = '';
...
export async function search(source, query, index, host) {
  ...
  if (searchLink) {
    ...
    if (template.includes('{searchTerms}')) { target = url(template.replaceAll('{searchTerms}', encodeURIComponent(query)).replace(/\{(?:startPage|page)\??\}/g, '1').replace(/\{[^}]+\?\}/g, ''), target); tree = await conn.fetch(target); filtered = false; }
  }
